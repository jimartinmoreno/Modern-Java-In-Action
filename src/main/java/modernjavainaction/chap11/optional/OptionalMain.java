package modernjavainaction.chap11.optional;
...
public class OptionalMain {
  ...
  /*public String getCarInsuranceName(Person person) {
    ...
    public Set<String> getCarInsuranceNames2(List<Person> persons) {
        return persons.stream()
                .flatMap(person -> person.getCar().stream()
                        .flatMap(car -> car.getInsurance().stream()
                                .map(Insurance::getName)))
                .collect(toSet());
    }
...
//        return person.map(p -> car.map(c -> findCheapestInsurance(p, c))).map(insurance -> insurance.get());
    }
